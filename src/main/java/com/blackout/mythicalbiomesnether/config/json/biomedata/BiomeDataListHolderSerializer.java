...
public class BiomeDataListHolderSerializer implements JsonSerializer<BiomeDataListHolder>, JsonDeserializer<BiomeDataListHolder> {
    ...
    public JsonElement serialize(BiomeDataListHolder Src, Type typeOfSrc, JsonSerializationContext context) {
        ...
        for (BiomeData biomeData : Src.getBiomeData()) {
            ...
            if (biomeData.getBiomeWeightedList() != null) {
                for (WeightedList.Entry<Biome> biomeEntry : biomeData.getBiomeWeightedList().entries) {
                    ...
                    if (biomeEntryKey != null) {
                        ...
                        MythicalBiomesNether.LOGGER.error("One or more \"hills\" \"name\" value was null/incorrect.");
                    }
                }
            }
            ...
            if (location != null )
                ...
                MythicalBiomesNether.LOGGER.error("The Biome key was null! This should NEVER happen.");
    ...
    public BiomeDataListHolder deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        MythicalBiomesNether.LOGGER.info("Reading json");
        ...
        for (Map.Entry<String, JsonElement> elementEntry : entrySet) {
            ...
            if (!defaultClimates.contains(climate) && !climate.isEmpty()) {
                MythicalBiomesNether.LOGGER.error(elementEntry.getKey() + "'s \"climate\" value is incorrect you put: \"" + climate + "\". Defaulting climate to warm...");
            ...
            if (types.size() == 0) {
                ...
                MythicalBiomesNether.LOGGER.warn("No dictionary entries were read...defaulting to: \"OVERWORLD\"");
            }
                ...
                if (hillBiomeName != null && hillWeight != null) {
                    ...
                    if (hillResourceLocation != null) {
                        if (WorldGenRegistries.BIOME.keySet().contains(hillResourceLocation))
                            ...
                            MythicalBiomesNether.LOGGER.error("Could not find: \"" + hillResourceLocation.toString() + "\" in the biome registry!\nEntry will not be added. Skipping entry...");
                    }
                }
            }
            ResourceLocation biomeKey = new ResourceLocation(biomeName);
            if (WorldGenRegistries.BIOME.keySet().contains(biomeKey)) {
                if (biomeKey.getNamespace().equals(MythicalBiomesNether.MOD_ID))
                    ...
                    MythicalBiomesNether.LOGGER.error("Biome key: \"" + biomeName + "\" is illegal. The mod id for the biome key MUST be \"nmb\". Skipping entry...");
            }
            else
                MythicalBiomesNether.LOGGER.error("The biome key: \"" + biomeName + "\" was not found in the registry, skipping entry...");
        }
    ...
    public String warnIfTagIsNotDefault(String string) {
        ...
        if (!defaultTypesList.contains(type) && !stopSpamLoggerSpam.contains(type)) {
            MythicalBiomesNether.LOGGER.warn(type.toString() + " is not a default dictionary value.\nIgnore this msg if using modded biome dictionary values.");

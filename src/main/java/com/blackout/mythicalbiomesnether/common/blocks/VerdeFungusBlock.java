...
import com.blackout.mythicalbiomesnether.common.world.feature.nether.mushrooms.util.MBNHugeMushroom;
...
public class VerdeFungusBlock extends BushBlock implements IGrowable {
    protected static final VoxelShape SHAPE = Block.box(5.0D, 0.0D, 5.0D, 11.0D, 6.0D, 11.0D);
    private final MBNHugeMushroom mushroom;

    private final boolean isNetherFungi;

    public VerdeFungusBlock(Properties properties, MBNHugeMushroom mushroom, boolean isNetherFungi) {
        super(properties);
    ...
    protected boolean mayPlaceOn(BlockState state, IBlockReader worldIn, BlockPos pos) {
        return (isNetherFungi) ? state.is(BlockTags.NYLIUM) || state.is(Blocks.SOUL_SOIL) || state.is(BlockTags.BASE_STONE_NETHER) || state.is(Tags.Blocks.NETHERRACK) || super.mayPlaceOn(state, worldIn, pos) : state.isSolidRender(worldIn, pos);
    }

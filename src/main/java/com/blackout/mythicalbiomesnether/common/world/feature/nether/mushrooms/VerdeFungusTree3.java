...
public class VerdeFungusTree3 extends MBNAbstractMushroomFeature<MBNMushroomConfig> {

    public VerdeFungusTree3(Codec<MBNMushroomConfig> configIn) {
        super(configIn);
    }
    ...
    protected boolean placeMushroom(ISeedReader worldIn, Random rand, BlockPos pos, boolean isMushroom, MBNMushroomConfig config) {
        ...
        if (pos.getY() + randTreeHeight + 1 < worldIn.getMaxBuildHeight()) {
            if (!isDesiredGroundwDirtTag(config, worldIn, pos.below(), MBNBlocks.VERDE_NYLIUM.get())) {
                ...
                if ((Math.random() >= 0.3)) {
                    this.hangingVerdeRootsPlant(worldIn, mainmutable.set(pos).move(0, 6, 2));
                    this.hangingVerdeRootsPlant(worldIn, mainmutable.set(pos).move(0, 5, 2));
                    ...
                    this.hangingVerdeRoots(worldIn, mainmutable.set(pos).move(0, 2, 2));
                }

                if ((Math.random() >= 0.3)) {
                    this.hangingVerdeRootsPlant(worldIn, mainmutable.set(pos).move(-1, 6, -2));
                    this.hangingVerdeRootsPlant(worldIn, mainmutable.set(pos).move(-1, 5, -2));
                    this.hangingVerdeRootsPlant(worldIn, mainmutable.set(pos).move(-1, 4, -2));
                    this.hangingVerdeRoots(worldIn, mainmutable.set(pos).move(-1, 3, -2));
                }
                ...
                if ((Math.random() >= 0.3)) {
                    this.hangingVerdeRootsPlant(worldIn, mainmutable.set(pos).move(1, 6, -2));
                    this.hangingVerdeRootsPlant(worldIn, mainmutable.set(pos).move(1, 5, -2));
                    this.hangingVerdeRoots(worldIn, mainmutable.set(pos).move(1, 4, -2));
                }

                if ((Math.random() >= 0.3)) {
                    this.hangingVerdeRootsPlant(worldIn, mainmutable.set(pos).move(0, 6, -2));
                    this.hangingVerdeRoots(worldIn, mainmutable.set(pos).move(0, 5, -2));
                }
            }
        }
        return true;

package com.blackout.mythicalbiomesnether.common.world.feature.nether.verdeforest;
...
public class VerdeStalk extends Feature<ProbabilityConfig> {
    ...
    public VerdeStalk(Codec<ProbabilityConfig> config) {
        super(config);
    }
    ...
    public boolean place(ISeedReader world, ChunkGenerator generator, Random rand, BlockPos pos, ProbabilityConfig config) {
        ...
        if (world.isEmptyBlock(mutable)) {
            if (MBNBlocks.VERDE_STALK_BLOCK.get().defaultBlockState().canSurvive(world, mutable)) {
                        ...
                        for (int z = pos.getZ() - randNextInt; z <= pos.getZ() + randNextInt; ++z) {
                            ...
                            if (xBuild * xBuild + zBuild * zBuild <= randNextInt * randNextInt) {
                                ...
                                if (isDirt(world.getBlockState(mutable2).getBlock())) {
                                    world.setBlock(mutable2, MBNBlocks.VERDE_NYLIUM.get().defaultBlockState(), 2);
                                }
                            }
                        }
                    }
    ...
    static {
        BAMBOO_BASE = MBNBlocks.VERDE_STALK_BLOCK.get().defaultBlockState().setValue(BambooBlock.AGE, 1).setValue(BambooBlock.LEAVES, BambooLeaves.NONE).setValue(BambooBlock.STAGE, 0);
